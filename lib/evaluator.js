        ...
        if (comparables && comparables.median) {
          ...
          if (comparables.listings && comparables.listings.length > 0) {
            ...
            comparables.listings.slice(0, 12).forEach((listing, i) => {
              ...
              // Format with clickable link (no target=_blank so overlay can follow navigation)
              if (listing.url) {
                comparablesText += `\n${i + 1}. <a href="${listing.url}" style="color: #4a9eff; text-decoration: none;">$${listing.price.toLocaleString()}</a> - ${details}`;
              } else {
